resource "aws_cloudfront_distribution" "cf_dist_botdr_web" {
  ...
  origin {
    ...
    origin_access_identity = aws_cloudfront_origin_access_identity.cf_oai_botdr_web.cloudfront_access_identity_path
  ...
  aliases = var.env == "dev" ? ["web${var.env}.battleofthedragonsrevived.com"] : ["battleofthedragonsrevived.com"]
